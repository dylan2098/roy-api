...
const RegisteredProductModel = require('./model')


const Status = Instance.getInstanceStatus();

router.get("/", async (req, res, next) => {
    const data = await RegisteredProductModel.getAll();
...
router.post('/', adminAuth, validate(schema), async (req, res) => {
    ...
    try {
        const data = await RegisteredProductModel.create(req.body);
...
router.delete('/', adminAuth, async (req, res) => {
    ...
    const isExist = RegisteredProductModel.isExist({ id: req.body.id });
    if (!isExist) {
        return Status.error(res, "Not found");
    }

    try {
        const data = await RegisteredProductModel.delete(req.body.id);
...
router.put('/', adminAuth, async (req, res) => {
    ...
    try {
        const data = await RegisteredProductModel.update(id, req.body);
